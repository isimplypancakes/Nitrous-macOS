...
///
/// `glassEffect` ships in iOS 26; the deployment target is 17, so every use is
/// gated and falls back to the closest material. Keeping the fallbacks here
/// means call sites stay declarative instead of littered with availability
/// checks.
extension View {
    ...
    func liquidGlass(tint: Color, cornerRadius: CGFloat = 22) -> some View {
        if #available(iOS 26.0, *) {
            ...
            self.background(tint.opacity(0.16),
                            in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
...
/// stacking separate blurs. No-op before iOS 26.
...
extension View {
    ...
    /// accent gradient so white text stays legible.
    @ViewBuilder
    func liquidGlassAccent(cornerRadius: CGFloat = 18) -> some View {
        if #available(iOS 26.0, *) {
            self.glassEffect(.regular.tint(Palette.accent).interactive(),
                             in: .rect(cornerRadius: cornerRadius, style: .continuous))
        } else {
            self.background(Palette.accent.gradient,
                            in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
        }
    }
}
...
extension View {
    /// A glass panel for the bottom compose bar — rounded only at the top so it
    /// merges into the safe-area edge, with a hairline top border. One surface
    /// for the reply bar + field, instead of stacked opaque panels.
    @ViewBuilder
    func liquidGlassBar() -> some View {
        ...
        // and would otherwise swallow every tap meant for the field or buttons.
        if #available(iOS 26.0, *) {
...
/// colour. Applied per screen because the tab/navigation containers paint an
/// opaque background over anything placed behind them at the root.
...
/// The springs used for press feedback. Tuned to feel like iOS 26's own glass
/// controls: a quick squash on touch-down, a slightly looser release.
enum Bounce {
...
/// Button style that squashes and lifts like a native glass control, with a
...
///
/// Prefer `.buttonStyle(.bouncy)`/`.bouncyRow`: UIKit cancels a button's press
        ...
        content
            ...
            .simultaneousGesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { v in
                        // Any real movement means the user is scrolling or
                        // swiping, not pressing — let go immediately so the
                        // scroll view takes over instead of the row staying
                        // squashed under the finger.
                        ...
                        pressed = true
                        if haptic { UIImpactFeedbackGenerator(style: .light).impactOccurred(intensity: 0.6) }
                    }
